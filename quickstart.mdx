---
title: "Get Started with Buildml in Five Minutes"
...
<Steps>
  ...
  <Step title="Configure environment variables">
    ...
    # Application URLs
    DEPLOYMENT_URL="http://localhost:3000"   # Public URL QStash posts callbacks to
    EXECUTOR_URL="http://localhost:8000"     # FastAPI executor endpoint
    EXECUTOR_SECRET="dev-secret"            # Shared secret for executor auth
    ```

    <Note>
      In `src/env.js` the `DEPLOYMENT_URL` value is read from `NEXT_PUBLIC_DEPLOYMENT_URL` at runtime. Set that variable (not just `DEPLOYMENT_URL`) if you need the value client-side as well. For local development a single `DEPLOYMENT_URL` entry in `.env` is sufficient.
    </Note>
  </Step>
  ...
  <Step title="Push the database schema">
    ...
    When you are ready to generate versioned migration files (for example, before a production deploy), use `db:migrate` instead:
