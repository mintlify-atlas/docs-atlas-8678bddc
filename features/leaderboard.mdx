---
title: "Community Leaderboard: Track Top Solvers"
sidebarTitle: "Leaderboard"
description: "The Buildml leaderboard ranks users by the number of unique AI/ML problems they have solved. Rankings update in real time as submissions are processed."
---
...
The `/leaderboard` page is publicly accessible — anyone can view the rankings. However, the **Personal Standing** banner and the **You** badge on table rows only appear for authenticated users. The `user.getUserRank` query is a `protectedProcedure` and is only called when a session exists:
