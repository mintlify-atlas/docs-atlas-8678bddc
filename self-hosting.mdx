---
title: "Deploy Buildml on Your Own Infrastructure"
sidebarTitle: "Self-Hosting"
description: "Run Buildml in production with PostgreSQL, Upstash Redis, Upstash QStash, Google OAuth, and a FastAPI executor service. Full environment variable reference."
---
...
<ParamField body="AUTH_SECRET" type="string" required>
  Random secret string used by NextAuth.js v5 to sign session tokens and cookies. In `src/env.js` this is read as `NEXTAUTH_SECRET`. **Required in production** — omitting it will cause a startup error.
...
<ParamField body="DEPLOYMENT_URL" type="string" required>
  The fully-qualified public URL of your Next.js application. QStash uses this to construct the callback URL when delivering webhook messages.

  ```
  DEPLOYMENT_URL="https://buildml.yourdomain.com"
  ```

  Note: in `src/env.js` this is mapped from `NEXT_PUBLIC_DEPLOYMENT_URL` at runtime.
</ParamField>
...
<Steps>
  ...
  <Step title="Set all environment variables">
    Copy `.env.example` to your deployment environment (Vercel environment settings, Docker secrets, etc.) and fill in every variable listed above. Do not leave any field blank in production.
  </Step>
  ...
  <Step title="Run database migrations">
    Apply the Prisma schema to your production PostgreSQL instance using a proper migration (not `db:push`):
  ...
  <Step title="Configure QStash webhook URL">
    ...
    This URL must match the value derived from `DEPLOYMENT_URL` in your environment.
  </Step>
...
<Warning>
  Protected routes (`/dashboard/*`, `/practice/*`, `/leaderboard`, `/profile/*`) are enforced by Next.js middleware. Ensure `AUTH_SECRET` is set correctly — an incorrect or missing value will silently break session validation and redirect authenticated users back to the sign-in page.
</Warning>
